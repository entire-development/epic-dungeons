...
void drawEntity(const std::shared_ptr<graphics::Renderer> &renderer,
    ...
    if (animations.find(entity->getId()) == animations.end()) {
        std::string path = "heroes/" + entity->getId() + "/idle.gif";
        animations[entity->getId()] = std::make_shared<graphics::Animation>(path);
    }
    auto sprite = animations[entity->getId()]->getFrame();
    if (position <= 3)
