...
std::string gui::dl::preprocessString(const std::string &str) {
    ...
    for (int i = 0; i < str.length(); i++) {
        if (str[i - 1] == '$')
            continue;   // preprocess ignore character.
        if (str[i] == ',') {
...
void DialogueWindow::drawQuote(std::shared_ptr<graphics::Renderer> renderer) {
    ...
    renderer->drawRec({
        .x = WINDOW_MARGIN,
        .y = cfg::WINDOW_HEIGHT - DIALOGUE_WINDOW_HEIGHT + WINDOW_MARGIN,
        .w = DIALOGUE_WINDOW_WIDTH - WINDOW_MARGIN * 2,
        .h = DIALOGUE_WINDOW_HEIGHT - WINDOW_MARGIN * 2,
        .color = graphics::Color("#000000"),
        .stroke = 5,
    ...
    for (int i = 0; i < m_content.size(); i++) {
        ...
        for (int j = 0; j < line.length(); j++) {   // I will explain this later
            ...
            // draw character
            if (display && current_len < m_current_index)
                renderer->draw(
                    graphics::Text(std::string(&line[j], &line[j + 1])).setColor(m_text_color).setFontSize(m_font_size),
                    WINDOW_MARGIN + PORTRAIT_SIZE + WINDOW_PADDING + char_pos * CHAR_WIDTH,
                    10 + cfg::WINDOW_HEIGHT - DIALOGUE_WINDOW_HEIGHT + WINDOW_MARGIN + WINDOW_PADDING
                        + LINE_HEIGHT * (i));
            if (m_current_index < m_content_len)
                current_len++;
            else {
                m_is_finished = true;
            }
...
void DialogueWindow::drawChoice(std::shared_ptr<graphics::Renderer> renderer, std::vector<std::string> lines,
    ...
    // CONTAINER
    renderer->drawRec({.x = WINDOW_MARGIN,
                       .y = cfg::WINDOW_HEIGHT - DIALOGUE_WINDOW_HEIGHT + WINDOW_MARGIN,
                       .w = DIALOGUE_WINDOW_WIDTH - WINDOW_MARGIN * 2,
                       .h = DIALOGUE_WINDOW_HEIGHT - WINDOW_MARGIN * 2,
                       .color = graphics::Color("#000000"),
        ...
        if (i == active_choice) {
            renderer->drawRec(
                {.x = static_cast<float>(WINDOW_MARGIN + WINDOW_PADDING - rec_padding),
                 .y = static_cast<float>(cfg::WINDOW_HEIGHT - DIALOGUE_WINDOW_HEIGHT + WINDOW_MARGIN + WINDOW_PADDING
                                         + LINE_HEIGHT * i - rec_padding),
...
DialogueManager::DialogueManager() :
    m_current_quote(nullptr), m_dialogue_window(DialogueWindow()), m_character_anim(gui::TimedCount()),
    m_is_active(false) {
...
void DialogueManager::setEntryPoint(script::ScriptNode *entry_point, gui::game::GameMachine *gm,
                                    std::function<void(gui::game::GameMachine *gm)> new_on_finish) {
...
void DialogueManager::prevChoice() {
    if (!m_is_dialogue)
        return;
    m_active_choice -= 1;
    if (m_active_choice + 1 == 0)
        m_active_choice = m_choice_lines.size() - 1;
}
...
void DialogueManager::handleKeyboard(gui::KeyboardManager &keyboard_manager, gui::game::GameMachine *gm) {
    ...
    if (keyboard_manager.isClicked(cfg::CONTROLS_MOVE_DOWN))
        this->nextChoice();
    if (keyboard_manager.isClicked(cfg::CONTROLS_MOVE_UP))
        this->prevChoice();
}
