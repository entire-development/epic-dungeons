...
DungeonMatrix::DungeonMatrix(size_t height, size_t width) :
...
bool DungeonMatrix::generate_room(int y, int x, bool trust) {
    ...
    if (trust) {
        if (get_cell(y, x) != DungeonMatrixCell::Empty)
            return false;
    } else {
        for (int dy = -2; dy <= 2; dy++)
            for (int dx = -2; dx <= 2; dx++)
                if (get_cell(y + dy, x + dx) != DungeonMatrixCell::Empty)
                    return false;
    }
...
DungeonMatrixCell DungeonMatrix::get_cell(int y, int x) const {
    if (y < 0 || x < 0 || y >= height || x >= width)
        return DungeonMatrixCell::Empty;
    return cells[y][x];
}
...
void DungeonMatrix::pave_horizontal(int y, int x1, int x2) {
    ...
    for (int x = x1; x <= x2; x++)
        if (get_cell(y, x) == DungeonMatrixCell::Empty)
            set_cell(y, x, DungeonMatrixCell::Corridor);
}
...
void DungeonMatrix::random_pave(int y1, int y2, int x1, int x2, pseudorandom::Randint &randint) {
    if (y1 == y2) {
        pave_horizontal(y1, x1, x2);
        return;
    }
    if (x1 == x2) {
        pave_vertical(y1, y2, x1);
        return;
    }
...
bool DungeonMatrix::check_errors(int y, int x, coords from) const {
    if (get_cell(y, x) != DungeonMatrixCell::Corridor)
        return false;
    ...
    for (coords n : neighbors(y, x)) {
        ...
        if (get_cell(n.first, n.second) == DungeonMatrixCell::Corridor) {
            ...
            if (get_cell(c1.first, c1.second) == DungeonMatrixCell::Room
                && get_cell(c2.first, c2.second) == DungeonMatrixCell::Room) {
                rooms_n_count++;
                continue;
            }
            ...
            if (get_cell(c1.first, c1.second) == DungeonMatrixCell::Room) {
                if (get_cell(y, c1.second) != DungeonMatrixCell::Corridor
                    && get_cell(c1.first, x) != DungeonMatrixCell::Corridor)
                    return true;
            } else {
                if (get_cell(y, c2.second) != DungeonMatrixCell::Corridor
                    && get_cell(c2.first, x) != DungeonMatrixCell::Corridor)
                    return true;
            }
        }
    }
    ...
    if (2 - rooms_n_count != corridors_n_count)
        return true;

    for (coords n : neighbors(y, x)) {
        if (n == from)
            continue;
        if (check_errors(n.first, n.second, coords(y, x)))
            return true;
    }
...
void DungeonMatrix::erase_room(int id) {
    ...
    for (coords n : neighbors(y, x, 2)) {
        ...
        for (coords c : *path)
            set_cell(c.first, c.second, DungeonMatrixCell::Empty);
    }
}
